...
export const Sidebar = () => {
  ...
  const displayName = state.isAuthenticated
  ...
  const sections = [
    ...
    },
    {
      title: "Tools",
      items: [
        { to: "/financer", label: "Financer", Icon: Wallet },
    ...
    <div className="flex h-full flex-col bg-white/95 dark:bg-slate-950/95 border-r border-gray-200/70 dark:border-white/10 shadow-sm shadow-slate-900/5">
      <div className="px-4 py-5 border-b border-gray-200/70 dark:border-white/10">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-3xl bg-black text-yellow-300 shadow-inner shadow-slate-900/30 overflow-hidden">
            {avatarUrl ? (
              <img src={avatarUrl} alt="User avatar" className="h-full w-full object-cover" />
            ) : (
              <span className="text-lg font-bold">{displayName?.[0]?.toUpperCase() || "F"}</span>
            )}
          </div>
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-slate-200/80 bg-slate-50 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-700 transition hover:bg-slate-100 dark:border-white/10 dark:bg-white/5 dark:text-text-300 dark:hover:bg-white/10"
      ...
      {state.isAuthenticated && (
        <div className="mt-auto border-t border-gray-200/70 dark:border-white/10 px-4 py-4">
          ...
          <p className="mt-3 text-xs leading-5 text-slate-500 dark:text-slate-400">
            This view keeps your financial info simple and easy to understand.
          </p>
        </div>
      )}
    </div>
